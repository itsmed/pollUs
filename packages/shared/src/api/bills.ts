...
export async function fetchBills(): Promise<BillsResponse> {
  const res = await fetch(`${getApiUrl()}/api/bill`);
  if (!res.ok) {
...
export async function fetchBillDetail(
  ...
  const res = await fetch(`${getApiUrl()}/api/bill/${congress}/${type.toLowerCase()}/${number}`);
  if (!res.ok) {
...
export async function fetchBillText(
  ...
  const res = await fetch(`${getApiUrl()}/api/bill/${congress}/${type.toLowerCase()}/${number}/text`);
  if (!res.ok) {
